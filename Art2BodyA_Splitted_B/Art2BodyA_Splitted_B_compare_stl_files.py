...
CHANGE LOG:
  • Volume readings now use trimesh consistently (was: numpy-stl in the
    "Volume comparison" section, trimesh in the "Symmetric difference"
    section — two different engines on the same file gave two different
    answers because numpy-stl integrates signed tetrahedra naively from
    raw STL triangles, while trimesh normalizes triangle winding on load.
    On meshes with many curved fillet faces, the STL exporter can emit a
    handful of inverted triangles which numpy-stl miscounts — visible as
    a ~500 mm³ undercount vs. the build123d & trimesh values).
...
BUILD123D_STL = os.path.join(BASE_DIR, "Art2BodyA_Splitted_B_G_1_28.stl")
FUSION_STL    = os.path.join(BASE_DIR, "Art2BodyA_Splitted_B_original.stl")
REPORT_TXT    = os.path.join(BASE_DIR, "Art2BodyA_Splitted_B_build123d_vs_original_G_1_28.txt")
...
# ════════════════════════════════════════════════════════════════════════════
# PART 1 — Mesh info  (uses trimesh — same engine as symmetric diff & bbox)
# ════════════════════════════════════════════════════════════════════════════
def load_and_inspect(filepath, label):
    ...
    print(f"  Triangles      : {len(m.faces):,}  (more = finer mesh = less approx error)")
    print(f"  Watertight     : {m.is_watertight}")
    print(f"  Volume         : {volume:.4f} mm³")
    print(f"  Center of mass : ({cog[0]:.3f}, {cog[1]:.3f}, {cog[2]:.3f}) mm")
    print(f"  X range        : {m.bounds[0,0]:.4f} → {m.bounds[1,0]:.4f} mm")
    print(f"  Y range        : {m.bounds[0,1]:.4f} → {m.bounds[1,1]:.4f} mm")
    print(f"  Z range        : {m.bounds[0,2]:.4f} → {m.bounds[1,2]:.4f} mm")
    return volume
